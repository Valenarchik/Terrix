...
namespace Terrix.Game.GameRules
{
    ...
    public class AttackInvoker : IAttackInvoker
    {
        ...
        private readonly HashSet<Attack> attacksSet = new();
            ...
            while (attackState.Queue.Count > 0 || attackState.Distances[attackState.Queue.Peek()] <= attackState.CurrentDistance)
            {
                ...
                foreach (var neighbour in cell.GetNeighbours(map))
                {
                    if (attackState.Distances.ContainsKey(neighbour) // если уже посещал
                        ...
                        || GetCellVirtualOwner(cell).PlayerId != attackState.Attack.Owner?.ID) // если не атакуем этого игрока
                    {
                        continue;
                    }
        ...
        {
            var startCells = attackState.Attack.Owner.Country.Border
                .SelectMany(hex => hex.GetNeighbours(map))
                .ToHashSet();
            
            startCells.ExceptWith(attackState.VirtualCountry.VirtualCellsSet);
